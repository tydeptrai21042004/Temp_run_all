...
def run_fashion_cnn(pde_op, out_dir: Path, batch_size=128, epochs=10, device=None, dtype=None, ablation_lrmp="lrmp"):
    train_loader, test_loader = get_fashionmnist_loaders(batch_size=batch_size)

    def build(): return FashionCNN()

    # Better baseline: include weight decay; keep lr sane
    lrs = (3e-3, 1e-3, 3e-4)
    wds = (1e-4, 5e-4)
    cfgs_common = [{"lr": lr, "weight_decay": wd} for lr in lrs for wd in wds]

    cfgs_lrmp = [{"lr": c["lr"], "weight_decay": c["weight_decay"], "rho0": r} for c in cfgs_common for r in (0.1, 0.3)]
    ...
    cfgs_lsgd = [{"lr": c["lr"], "weight_decay": c["weight_decay"], "alpha": a} for c in cfgs_common for a in (0.25, 0.5, 0.75)]
    ...
    def make_lsgd(model, cfg):  return AdamW_GradSmooth(model.parameters(), lr=cfg["lr"], weight_decay=cfg.get("weight_decay", 0.0), alpha=cfg.get("alpha", 0.5))
    def make_lrmp(model, cfg):  return AdamW_LRMP(model.parameters(), lr=cfg["lr"], weight_decay=cfg.get("weight_decay", 0.0), rho0=cfg.get("rho0", 0.3), rho_mode=ablation_lrmp)
    ...
    for name, make_opt, cfgs in methods:
        ...
        best_cfg = quick_tune_classifier(build, make_opt, train_loader, test_loader, cfgs, name,
                                         device=device, dtype=dtype, tune_batches=120, eval_batches=80)
    ...
    plot_3d_loss_curves(histories, out_dir, title="FashionMNIST CNN - train loss", key="train_loss", filename="loss3d_train.png")
    plot_3d_loss_curves(histories, out_dir, title="FashionMNIST CNN - test loss",  key="test_loss",  filename="loss3d_test.png")
